...
RESULTS_DIR = 'tests/output/jsons'
makedirs(RESULTS_DIR, exist_ok=True)
GOOGLE_API_KEY = os.environ['GOOGLE_API_KEY']


class GoogleVisionApis:
    ...
    def request_translation(self, text_list, target_language='en'):
        response = None
        try:
            service = build("translate", "v2", developerKey=GOOGLE_API_KEY)
            collection = service.translations()
            req = collection.list(q=text_list, target=target_language)
...
if __name__ == '__main__':
    ...
    if not api_key or not image_filenames:
        ...
        response = GoogleVisionApis().request_ocr(image_filenames)
